...
| `Kcte` | 0.0 | クロストラックエラー比例ゲイン。横偏差(m)に乗算 |
| `Ki_cte` | 2.0 | クロストラックエラー積分ゲイン。積分値(m*s)に乗算 |
| `look_ahead` | 0.0 | ルックアヘッド距離 (m) |
| `pivot_threshold` | 40.0 | ピボットターンに切り替わるヨー誤差の閾値 (度) |
| `cte_threshold` | 0.1 | CTE比例補正が有効になる横偏差の閾値 (m) |
| `cte_i_limit` | 1.5 | CTE積分値の上限 (m*s) |
| `cte_i_active` | 0.8 | CTE積分が有効になる横偏差範囲 (m) |
| `cte_i_decay` | 0.98 | 積分停止時に各周期で掛ける減衰率 |
